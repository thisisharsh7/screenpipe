...
pub async fn list_audio_devices() -> Result<Vec<AudioDevice>> {
    ...
    #[cfg(not(all(target_os = "linux", feature = "pulseaudio")))]
    {
        ...
        #[cfg(target_os = "macos")]
        {
            // On macOS 14.4+, CoreAudio Process Tap is always available for
            // system audio — no display enumeration needed.
            if super::process_tap::is_process_tap_available() {
                tracing::debug!(
                    "Process Tap available — listing '{}' without SCK display check",
...
pub async fn default_output_device() -> Result<AudioDevice> {
    ...
    #[cfg(target_os = "macos")]
    {
        // On macOS 14.4+, CoreAudio Process Tap provides reliable system audio
        // capture without needing SCK display enumeration.
        if super::process_tap::is_process_tap_available() {
