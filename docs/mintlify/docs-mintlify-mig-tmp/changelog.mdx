...
description: "What's new in screenpipe — releases, features, and fixes."
icon: "newspaper"
---
...
- **Zoom meetings no longer end prematurely** — when you minimize Zoom, switch apps, or share your screen, [meeting capture](/meeting-transcription) now stays active as long as audio is flowing, instead of cutting out after 30 seconds.
- **no more phantom Zoom meetings** — Zoom is no longer flagged as "in a meeting" when the app is just idle in the background.
- **fewer duplicate speakers** — speaker diarization now seeds against all recent speakers, eliminating cases where the same voice was split across hundreds of unnamed entries.
- **local network discovery stability** — long machine hostnames no longer crash the mDNS service used for local-network features.
